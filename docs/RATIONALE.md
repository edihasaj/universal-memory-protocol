...
- **"Open Memory Protocol" (EB DevTech, 2026)** — a *different problem*, same
  would-be name: **conversation-transcript backup/portability** ("the iCloud of
  AI"). Core unit is a `Conversation`/`Message`; "5 operations" are
  Backup/Restore/Migrate/Delete/Search over a `.omp.zip` archive; its Memory
  Record is secondary (single `supersedes`, no bi-temporal, no provenance graph,
  no signing, no MCP binding). Solo, no traction at time of survey. *This is why
  we are NOT "Open Memory Protocol":* the name is taken by a transcript-backup
  effort, and our problem (the live agent-memory runtime) is a different layer.
...
runtime** with an **MCP binding** — which none of the four have. (SPEC §4.1.)
